...
// FORK(hollowed-oath) BEGIN: signed realm pin. See RedwoodSettings.h.
void URedwoodSettings::GetSignedRealm(
  FString &OutRealmId, FString &OutRealmUri
) {
  OutRealmId.Reset();
  OutRealmUri.Reset();
  ...
  ReadSignedRealm(
    FPaths::ProjectDir() / TEXT("redwood.json"),
    Settings->PublicSigningKey,
    OutRealmId,
    OutRealmUri
  );
}

void URedwoodSettings::ReadSignedRealm(
  ...
  TSharedPtr<FJsonObject> JsonObject =
    LoadSignedRedwoodConfig(JsonPath, PublicSigningKeyBase64);
  if (!JsonObject.IsValid()) {
    return;
  }

  JsonObject->TryGetStringField(TEXT("realmId"), OutRealmId);
  JsonObject->TryGetStringField(TEXT("realmUri"), OutRealmUri);
}
